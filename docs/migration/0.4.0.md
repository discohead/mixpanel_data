...
> Mirrors the canonical [`RELEASE_NOTES_0.4.0.md`](https://github.com/mixpanel/mixpanel-headless/blob/main/RELEASE_NOTES_0.4.0.md) — update both together.

`mixpanel_headless` 0.4.0 ships a complete rewrite of the auth subsystem (single TOML schema, three first-class account types, a single resolver, fluent in-session switching). It is a **hard break** from 0.3.x: legacy `~/.mp/config.toml` files no longer load and there is no auto-migration path. Recovery is a wipe + re-add (5 commands).
...
The following names were **deleted** from `mixpanel_headless`:
...
The following names were **added** as part of the v3 surface and are canonical going forward (re-exported from the top-level `mixpanel_headless` namespace):
...
The following low-level types are available only from `mixpanel_headless.auth_types`:
...
- The `mixpanel-headless` Claude Code plugin manifest version bumps from `4.1.0` → `5.0.0`.
- The `/mixpanel-headless:auth` slash command re-routes to the new subcommand vocabulary (`account / project / workspace / target / session / bridge`). Old verbs (`status / list / switch / migrate / context / cowork-setup`) are gone.
...
Pin `mixpanel-headless >= 5.0.0` so the slash command consumes the new JSON contract.
